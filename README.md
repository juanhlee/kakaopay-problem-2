...
소스 상으로는 TYPE이 PAYMENT인 레코드가, 다수의 TYPE가 CANCEL인 레코드를 갖는 1:N 형태로 관리되도록 했음.<br/>
...
아래는 날짜별로 브랜치로 구성되어 있음.
사전 과제 당시에 제출했던 브랜치와 이후 추가 보완한 브랜치를 따로 구성함.
master 브랜치가 가장 최신 상태를 가지도록 했음.
